...
export default function ExpenseCategoriesPieChart({
  ...
  useEffect(() => {
    async function fetchData() {
      ...
      const [expRes, catRes] = await Promise.all([
        supabase
          .from("expenses")
          .select("category_id, total_cost, expense_date"),
  ...
  const chartData = useMemo(() => {
    ...
    filteredExpenses.forEach((exp) => {
      ...
      categoryTotals[catId] =
        (categoryTotals[catId] || 0) + (Number(exp.total_cost) || 0);
    });
  ...
  return (
    <div className="relative col-span-1 flex h-full min-h-100 flex-col gap-5 rounded-xl border border-gray-200 bg-white p-5 shadow-sm md:col-span-2">
      {isLoading && (
      ...
      {/* CHART CONTENT */}
      {chartData.length > 0 ? (
        <div className="mt-2 flex w-full flex-1 flex-col items-center justify-between gap-8 md:flex-row md:items-stretch">
          {/* Pie Chart */}
          <div className="relative min-h-70 w-full shrink-0 md:w-1/2">
                ...
                return (
                  ...
                  >
                    ...
                    <div className="flex items-center gap-4">
                      ...
                      <span className="min-w-[32px] text-right text-gray-500">
                        {percentage}%
                      </span>
                    </div>
                  </li>
        ...
        <div className="flex min-h-70 flex-1 flex-col items-center justify-center gap-3 text-gray-500">
