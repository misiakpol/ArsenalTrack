...
export default function RecentTransactionsTable() {
  ...
  const handleEdit = (transaction: any) => {
    ...
    setEditForm({
      ...
      total_cost: transaction.total_cost || (transaction.unit_cost * transaction.quantity).toString(),
  ...
  return (
    <div className="bg-white border border-gray-200 rounded-xl shadow-sm flex flex-col relative col-span-1 md:col-span-2 lg:col-span-3">
      {isLoading && (
                    ...
                    <tr key={tx.id} className="bg-purple-50/30">
                      <td className="px-6 py-3">
                        <input 
                          ...
                          className="w-full px-2 py-1 text-sm border border-gray-300 rounded focus:outline-none focus:border-purple-500"
                        />
                      </td>
                      <td className="px-6 py-3">
                        <input 
                      ...
                      <td className="px-6 py-3">
                        <input 
                          ...
                          className="w-full px-2 py-1 text-sm border border-gray-300 rounded focus:outline-none focus:border-purple-500 text-right"
                        />
                      </td>
                      <td className="px-6 py-3">
                        <input 
                ...
                return (
                  <tr key={tx.id} className="hover:bg-gray-50/50 transition-colors group">
                    <td className="px-6 py-4 whitespace-nowrap font-medium text-gray-700">
                      {new Date(tx.expense_date).toLocaleDateString("pl-PL", { day: '2-digit', month: '2-digit', year: 'numeric' })}
                    </td>
                    ...
                    <td className="px-6 py-4 whitespace-nowrap text-right font-bold text-red-600">
                      -{Number(amount).toLocaleString("pl-PL", { minimumFractionDigits: 2, maximumFractionDigits: 2 })} zł
                    </td>
                    <td className="px-6 py-4 text-gray-600">
                      <div className="flex items-center gap-2">
                        {/* Fake icon for store/method matching design */}
                        <div className="w-5 h-5 rounded-full bg-gray-100 flex items-center justify-center shrink-0">
                          <span className="text-[10px] text-gray-500 font-bold">{tx.store ? tx.store.charAt(0).toUpperCase() : '?'}</span>
                        </div>
                        <span className="truncate max-w-[120px]">{tx.store || "Unknown"}</span>
                      </div>
                    </td>
      ...
      {transactions.length > 10 && (
        <div className="p-4 border-t border-gray-100 flex justify-center bg-gray-50/50 rounded-b-xl">
          <button 
            onClick={() => setShowAll(!showAll)}
            className="text-purple-600 font-semibold text-sm hover:text-purple-700 hover:underline transition-all"
          >
